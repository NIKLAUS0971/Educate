...
export const GenderOfTheTeacher = ({ data }: any) => {
    ...
    const [item, setSelectItem] = useState("Müəllimin cinsi")
                ...
                </div>
                {
                    isActive && (
                        <div className="dropdown-content drob_down_content_new_class" style={{ width: '100%', marginBottom:'24px'}}>
                            {dataForCheckBox[7].map((option, index) => (
                                <option
                                    style={{cursor:'pointer'}}
                                    key={option.id} value={option.id}  onClick={(e:any) => {setSelectItem(option.name); setIsActive(false); HandleSelectGenderOfTheTeacher(option.id)}
